...
TraceCwnd (std::string cwnd_tr_file_name)
{
  ...
  //std::cout << cwnd_tr_file_name << std::endl;
...
int main (int argc, char *argv[])
{
  ...
  std::cout << "tcp portocol: " << transport_prot << std::endl; 
  ...
  //std::cout << "IP Header size is: " << ip_header << std::endl;
  delete temp_header;
  temp_header = new TcpHeader ();
  uint32_t tcp_header = temp_header->GetSerializedSize ();
  NS_LOG_LOGIC ("TCP Header size is: " << tcp_header);
  //std::cout << "TCP Header size is: " << tcp_header << std::endl;
  delete temp_header;
  ...
  sources.Create (num_flows); // source node
  NodeContainer sinks;
  sinks.Create (num_flows); // server node
  ...
  PointToPointHelper UnReLink; //error model for server ndoe
  ...
  PointToPointHelper LocalLink; // client node
